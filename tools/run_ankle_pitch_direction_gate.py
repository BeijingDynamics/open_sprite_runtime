#!/usr/bin/env python3
"""Run a small bidirectional pitch-response gate on one unloaded ankle."""
...
def main() -> None:
    ...
    parser.add_argument("--tested-foot-unloaded", action="store_true")
    ...
    with SocketCanMotorGroupMitWriter.open(
        ...
        gate = run_ankle_pitch_direction_gate(
            writer,
            endpoints,
            pair,
            side=args.side,
    ...
    if not report["passed"]:
        raise SystemExit(f"{args.side} ankle pitch-direction gate failed closed")
