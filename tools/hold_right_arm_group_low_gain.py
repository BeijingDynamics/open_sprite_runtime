#!/usr/bin/env python3
"""Hold the exact seven Sprite0825 right-arm motors at measured positions."""
...
ACKNOWLEDGEMENT = "ENABLE_RIGHT_ARM_GROUP_LOW_GAIN_HOLD"
...
def main() -> None:
    ...
    names = tuple(item[0] for item in RIGHT_ARM_GROUP)
    ...
    for endpoint, expected, model, sign in zip(
        endpoints, RIGHT_ARM_GROUP, expected_models, expected_signs, strict=True
    ):
        ...
        if (
            ...
            raise SystemExit(f"frozen right-arm invariant failed for {endpoint.motor_name}")
    ...
    with SocketCanMotorGroupMitWriter.open(
        ...
        hold = run_right_arm_group_low_gain_hold(
            writer, endpoints, soft_position_rad=soft_limits
        )
    report = {
        "mode": "first_powered_fixed_right_arm_current_position_hold",
        "group": "right_arm_seven_motors",
    ...
    if not report["passed"]:
        raise SystemExit("right-arm group low-gain hold failed closed")
