#!/usr/bin/env python3
"""Read PMAX/VMAX/TMAX from disabled Damiao drives using only RID 21/22/23."""
...
from open_sprite_runtime.damiao_registers import collect_mit_range_registers
...
CONFIRMATION = "READ_ONLY_DAMIAO_MIT_RANGES"
...
def main() -> None:
    ...
    if not args.supported_unloaded:
        raise SystemExit("--supported-unloaded is required")
    ...
    with ExitStack() as stack:
        readers = {
            interface: stack.enter_context(
                SocketCanDamiaoRegisterReader.open(
                    interface,
                    preflights[interface],
                    (endpoint.can_id for endpoint in by_interface[interface]),
                )
            )
            for interface in interfaces
        }
        report = collect_mit_range_registers(
