...
if [[ "${1:-}" != "ENABLE_RIGHT_ARM_GROUP_LOW_GAIN_HOLD" ]]; then
  echo "FIXED MOTOR-GROUP HOLD: right-arm seven motors only" >&2
  echo "Robot must be supported; the complete right arm and cables must be clear;" >&2
  ...
  echo "  $0 ENABLE_RIGHT_ARM_GROUP_LOW_GAIN_HOLD" >&2
  exit 2
fi

ROOT=/home/tony/open_sprite_runtime
STAMP="$(date +%Y%m%d_%H%M%S)"
SNAPSHOT="$ROOT/reports/kcan4_right_arm_group_hold_${STAMP}.json"
OUTPUT="$ROOT/reports/right_arm_group_low_gain_hold_${STAMP}.json"

mkdir -p "$ROOT/reports"
ip -j -d link show type can > "$SNAPSHOT"

echo "ACTIVE MOTOR-GROUP HOLD: exactly seven right-arm motors"
echo "BUS kcan4; IDs 0x01..0x07"
...
exec env PYTHONPATH="$ROOT/src" "$ROOT/.venv/bin/python" \
  "$ROOT/tools/hold_right_arm_group_low_gain.py" \
  ...
  --right-arm-clear \
  --confirm-hardware-tx ENABLE_RIGHT_ARM_GROUP_LOW_GAIN_HOLD
